...
async def wiki(ctx):
    ...
    if ctx.invoked_subcommand is None:
        embed = discord.Embed(
            title = "Invalid wiki command passed",
            description = "Enter *.wiki help* to see WikiWally commands.",
            color = 0xFAD02C
            )
...
async def get_page(ctx, *, user_input):
    ...
    try:
        ...
        embed = discord.Embed(
            title = page.title, 
            url = page.url,
            description = wikipedia.summary(query, sentences=2, 
                                            auto_suggest=False),
            color= 0xFAD02C
            )
        ...
        # Occurs when a Wikipedia page cannot be found given the user input.
        # The actual error thrown is IndexError, however this results from a
        # PageError: Because user inputs are being susbtituted with the first
        # query from wikipedia.search(user_input), an IndexError is thrown if
        # no pages for that user_input can be found.

        page_err_msg = "The page you are looking for might not exist. \
        ...
        # Occurs when multiple Wikipedia pages are found for a given query.
        ...
        embed = discord.Embed(
                title="There may be multiple entries for your query",
...
random_help = "Fetches a random Wikipedia article. An integer from 1 to 10 \
               can also be entered for a list of 1 to 10 random Wikipedia \
               articles (e.g. *.wiki random 10)*."
@wiki.command(name="random", help=random_help)
async def get_random_page(ctx, user_input=None):
    ...
    try:
        ...
        # embed similar to that of the "page" command.
            ...
            embed = discord.Embed(
                    title = page.title, 
                    url = page.url,
                    description = wikipedia.summary(page.title, sentences=2,
                                                    auto_suggest=False), 
                    color= 0xFAD02C
                    )
        ...
        # A list of articles on an embed are returned to the user.
            ...
            for index, query in enumerate(page_list):
                try:
                    ...
                    embed.add_field(
                        name="Article " + index_string, 
                        value="[{}]({})".format(page.title, page.url),
                        inline=False
                        )
    ...
    # Thrown when the numerical input is not a valid integer.
        ...
        val_err_msg = "Please input an integer from 1 to 10. \
                       If no number is entered, I'll send one \
                       article by default."

        embed = discord.Embed(
                title = "The query \"{}\" is not valid".format(user_input),
    ...
    # Thrown when the input cannot be converted into an integer.
        ...
        type_err_msg = "Please input an integer from 1 to 10. \
                       If no number is entered, I'll send one \
                       article by default."
...
options_help = "Look for query options/suggestions for a given input. \
                This may be helpful if you can't find the article \
                you're looking for using the *page* command."
@wiki.command(name="options", help=options_help)
async def get_options(ctx, user_input):
    ...
    # Triggers if there are no possible options for the user input
        ...
        page_err_msg = "Articles for your query might not exist. \
                        Please try a different query!"
...
async def get_help(ctx):
    """Returns the names and functions of WikiWally subcommands
    in an embed."""

    embed = discord.Embed(
            title = "WikiWally Bot Commands",
            description = "To use WikiWally Commands, \
                           enter ***.wiki [command name]***", 
            color = 0xFAD02C
            )
